...
smd({
  ...
  try {
    ...
    if (url.toLowerCase().includes("https")) {
      ...
      return await message.send("`Allen Bot V2 UPDATE WAS SUCCESSFULL ✅`");
    } else {
...
astro_patch.smd({
  ...
  try {
    ...
    let menuText = `
${menuThemeHeader}
${menuThemeCommandPrefix} *𝐏𝐎𝐖𝐄𝐑𝐄𝐃 𝐁𝐘:* ${Config.ownername}
${menuThemeCommandPrefix} *ᴜᴘᴛɪᴍᴇ:* ${runtime(process.uptime())}
${menuThemeCommandPrefix} *𝐒𝐄𝐑𝐕𝐄𝐑 𝐑𝐀𝐌:* ${formatp(os.totalmem() - os.freemem())}
...
astro_patch.smd({
  ...
  await context.send(`*🐣...latency of Allen Bot V2 is....🐤: ${pingTime} ᴍs*`, {
    edit: messageKey
  }, "", context);
});
astro_patch.smd({
    ...
    const endTime = new Date().getTime();
    const pingTime = endTime - startTime;
    await context.send(`*ALLEN IS ACTIVE....🐤*`, {
      edit: messageKey
    }, "", context);
  });
smd({
...
smd({
  ...
  try {
    ...
    const contextInfo = {
      isForwarded: true,
      forwardingScore: 999,
      title: "Allen Bot V2 Running Since",
      body: `*Allen Bot V2 Running Since:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`,
      footerText: "Allen Bot V2 || Beauty Ai",
      isSendNotificationMsg: true,
      mentionedJid: []
    };
    await m.bot.sendMessage(m.chat, {
      text: `*Allen Bot V2 Running Since:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`,
      contextInfo
    }, {
      quoted: null
    });
...
smd({
  ...
  try {
    ...
    if (!isNaN(_0x56b5d3)) {
      if (_0x1eb88a.length > 30 && _0x1eb88a[1].includes("Allen Bot V2_FANCY_TEXT")) {
      ...
      if (_0x56c097.length > 0 && _0x56c097.length < 20) {
        ...
        for (const _0x1af79d in _0x59e793) {
          ...
          if (_0x56c097.includes(_0x37f2ac)) {
            _0x5cca14 = "┏━━━━━━━━━━━━━━━━━━━━━━━\n┃\tAllen Bot V2_" + _0x1af79d.toUpperCase() + "_COMMANDS*  \n┗━━━━━━━━━━━━━━━━━━━━━━━\n\n\n";
